...
public class ProjectorScreen extends GuiLayer {
    ...
    public void create() {
        ...
        inputUrl = new WidgetTextField(buttonSave, DisplayData.URL, projector.getUrl()).setSuggest("https://i.imgur.com/1yCDs5C.mp4");
        ...
        parentSize.add(new WidgetCounterDecimal("width", projector.getSizeX(), 0, FrameConfig.maxWidth(), scaleMultiplier)
                .expandX()
                .setSpacing(0)
                .setAlign(Align.CENTER)
                .add2(new GuiButton("reX", 16, 16, but -> {
        ...
        parentSize.add(new WidgetCounterDecimal("height", projector.getSizeY(), 0, FrameConfig.maxHeight(), scaleMultiplier)
                .expandX()
                .setSpacing(0)
                .setAlign(Align.CENTER)
                .add2(new GuiButton("reY", 16, 16, but -> {
                    WidgetCounterDecimal sizeXField = get("width", WidgetCounterDecimal.class);
                    WidgetCounterDecimal sizeYField = get("height", WidgetCounterDecimal.class);
        ...
        parentSize.add(new WidgetParent(GuiFlow.STACK_Y)
                .add2(new GuiCheckBox(DisplayData.FLIP_X, projector.data.flipX).setTranslate("gui.waterframes.flipx"))
                .add2(new GuiCheckBox(DisplayData.FLIP_Y, projector.data.flipY).setTranslate("gui.waterframes.flipy")));
        ...
        WidgetDoubleTable parentTexSettings = new WidgetDoubleTable(GuiFlow.STACK_Y).setSpacing(2).expandY()
                .addOnFirst(new WidgetParent(GuiFlow.STACK_X)
                        ...
                        .add2(new WidgetSlider(DisplayData.ROTATION, 130, 10, projector.data.rotation, 0, 360, WidgetSlider.ANGLE)))
                .addOnFirst(new WidgetParent(GuiFlow.STACK_X)
                        .add2(new WidgetIcon("t_icon", 12, 12, CustomIcons.TRANSPARENCY))
                        .add2(new WidgetSlider(DisplayData.ALPHA, 130, 10, projector.data.alpha, 0, 1, WidgetSlider.PERCENT)))
                .addOnFirst(new WidgetParent(GuiFlow.STACK_X)
                        .add2(new WidgetIcon("b_icon", 12, 12, CustomIcons.BRIGHTNESS))
                        .add2(new WidgetSlider(DisplayData.BRIGHTNESS, 130, 10, projector.data.brightness, 0, 1, WidgetSlider.PERCENT)))
                .addOnFirst(new WidgetParent(GuiFlow.STACK_X)
                        .add2(new WidgetIcon("d_icon", 12, 12, CustomIcons.DISTANCE))
                        .add2(new GuiSteppedSlider(DisplayData.RENDER_DISTANCE, 130, 10, projector.data.renderDistance, 5, 1024)))
                .addOnFirst(new WidgetParent(GuiFlow.STACK_X)
        ...
        parentMedia.addOnFirst(new WidgetParent("", GuiFlow.STACK_X)
                ...
                .addOnSecond(new WidgetParent()
                        .add2(new GuiStateButton(ProjectorData.AUDIO_OFFSET, projector.data.getOffsetMode(), new TextListBuilder()
                                .addTranslated("gui.waterframes.audiocenter.", "block", "between", "projection")))
                        .setAlign(Align.RIGHT))
                .addOnSecond(new WidgetParent("", GuiFlow.STACK_X)
                        .add2(new WidgetIcon("v_icon", 12, 12, CustomIcons.getVolumeIcon(projector.data.volume)))
                        .add2(new WidgetSlider(DisplayData.VOLUME, 130, 10, projector.data.volume, 0, FrameConfig.maxAudioVolume(), WidgetSlider.PERCENT).setExpandableX())
                        .setAlign(Align.RIGHT))
                .addOnSecond(new WidgetParent("", GuiFlow.STACK_X)
                        .add2(new WidgetIcon("v_min_icon", 12, 12, CustomIcons.VOLUME_RANGE_MIN))
                        .add2(rangeMin = (GuiSteppedSlider) new GuiSteppedSlider(DisplayData.VOL_RANGE_MIN, 63, 10, projector.data.minVolumeDistance, 0, Math.min(FrameConfig.maxAudioDistance(), projector.data.maxVolumeDistance)).setExpandableX())
                        .add2(new WidgetIcon("v_max_icon", 12, 12, CustomIcons.VOLUME_RANGE_MAX))
                        .add2(new WidgetSteppedSlider(DisplayData.VOL_RANGE_MAX, rangeMin, 63, 10, projector.data.maxVolumeDistance, 0, FrameConfig.maxAudioDistance()).setExpandableX())
                .setAlign(Align.RIGHT));
        ...
        this.add(new WidgetSeekBar("seek", 150, 12, projector.data.tick, 0, projector.display != null ? projector.display.durationInTicks() : 1, () -> projector.data.tick)
                ...
                .addOnMouseRelease(seekBar -> FrameNet.syncPlaybackState(projector.getBlockPos(), projector.data.playing, projector.data.tick = (int) seekBar.value))
                .setExpandableX());
