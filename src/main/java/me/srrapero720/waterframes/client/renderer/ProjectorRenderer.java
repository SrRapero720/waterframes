...
@OnlyIn(Dist.CLIENT)
public class ProjectorRenderer implements BlockEntityRenderer<BEProjector> {
    @Override
    public boolean shouldRenderOffScreen(BEProjector frame) {
        return frame.getSizeX() > 16 || frame.getSizeY() > 16;
    }
    
    @Override
    public boolean shouldRender(BEProjector frame, Vec3 vec) {
        return Vec3.atCenterOf(frame.getBlockPos()).closerThan(vec, frame.renderDistance);
    }
    
    @Override
    public void render(BEProjector projector, float partialTicks, PoseStack pose, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        if (projector.isURLEmpty() || projector.alpha == 0) {
            if (projector.display != null) projector.display.release();
            return;
        }
        
        Display display = projector.requestDisplay();
