...
public class VideoDisplay extends Display {
    ...
    public int getVolume(float volume, float minDistance, float maxDistance) {
        ...
        float distance = (float) pos.distance(Minecraft.getInstance().player.getPosition(WaterFrames.toDeltaFrames()));
            ...
            if (stream) {
                ...
                if (player.getDuration() > 0) {
                    ...
                    if (player.isSeekable()) {
                        long time = tick * tickTime + (realPlaying ? (long) (WaterFrames.toDeltaFrames() * tickTime) : 0);
