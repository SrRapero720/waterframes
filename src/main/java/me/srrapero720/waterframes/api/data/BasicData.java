...
import me.srrapero720.waterframes.custom.screen.widgets.WidgetCounterDecimal;
...
public abstract class BasicData {
    ...
    public static <D extends BasicData, T extends BasicBlockEntity<?>> void sync(T block, Player player, CompoundTag nbt, ExtraData<D> extra) {
        ...
        if (WaterConfig.canUse(player, url)) {
            ...
            block.data.maxVolumeDistance = Math.min(WaterConfig.maxAudioDistance(), nbt.getInt(VOL_RANGE_MAX));
    ...
    public interface ExtraData<T extends BasicData> {
        void set(T data);
    }
}